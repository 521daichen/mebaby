...
//        print_r($billInfo);
     ...
     * @return bool
     */
    public function specialMemberDel($uid){
        if($uid!=''){
            $userinfo=pdo_fetch('select cardsn from ims_mc_card_members where uid="'.$uid.'"');
            $cardTypeName=substr($userinfo['cardsn'],0,1);
            switch ($cardTypeName)
            {
                //高级会员
                case '8':
    ...
    public function doMobileBindPlateNumberOnlyOne()
    {
        ...
        if ($_W['ispost'] && $_W['isajax']) {
            ...
            $isSeniorCustomer = $this->specialMemberDel($uid);
...
//	}
    ...
    {
		header('location:http://wx.cnsaga.com/app/index.php?i=4&c=entry&eid=1&wxref=mp.weixin.qq.com#wechat_redirect');
    //    $user_agent = $_SERVER['HTTP_USER_AGENT'];
    //    if (strpos($user_agent, 'MicroMessenger') === false) {
    //        // 非微信浏览器禁止浏览
    ...
    public function doMobilestartsyn(){

        $str = '20150426213977949,
20150316142233321,
20141012151217622,
20150403135146090,
20141209142617726,
...
20170507235786223';
        ...
        foreach($res1 as $key=>$custId){
            ...
            if(substr($cardinfo['cardId'],0,1) == '8'){
                $updateRes = $this->updateErpToLocal($cardinfo);
                echo $updateRes;
            }
        }
...
//        var_dump($arr1);
    ...
    public function updateErpToLocal($userInfo){
        $custId = $userInfo['custId'];
        $cardsn = $userInfo['cardId'];

        $data = array(
            'cardsn'=>$cardsn
        );
        $res = pdo_update('mc_card_members',
            $data,
            array('custId'=>$custId)
        );

        if($res) {
            return '更新成功'.$custId.'<br \>';
        }else{
            return '更新失败'.$custId.'<br \>';
        }
    ...
    public function getErpInfoToLocal($memberInfo){
    ...
    public function doMobileMobIndex(){
        include $this->template('mobindex/index');
    }
