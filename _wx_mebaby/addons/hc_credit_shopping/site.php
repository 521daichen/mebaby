...
//			}
	...
	public function doMobileMyOrder() {
		...
		if(empty($op) || $op == 2){
			$op = 2;
			$order = pdo_fetchall( " SELECT * FROM ".tablename('hc_credit_shopping_order')." WHERE from_user='".$_W['openid']."' AND weid='".$_W['uniacid']."' AND paytype !=9 AND status >=1 AND status <=2 " );
		}
		if($op == 1){
			$order = pdo_fetchall( " SELECT * FROM ".tablename('hc_credit_shopping_order')." WHERE from_user='".$_W['openid']."' AND weid='".$_W['uniacid']."' AND paytype !=9  " );
		}
		if($op == 3){
			$order = pdo_fetchall( " SELECT * FROM ".tablename('hc_credit_shopping_order')." WHERE from_user='".$_W['openid']."' AND weid='".$_W['uniacid']."' AND paytype !=9 AND status=3  " );
		}
	...
	*/
	...
	public function doMobilepay_way(){
		...
		//检查有没有填写地址
		$check_address = pdo_fetchcolumn( " SELECT id FROM ".tablename('hc_credit_shopping_address')." WHERE openid='".$_W['openid']."' AND weid='".$_W['uniacid']."' " );
