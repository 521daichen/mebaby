...
<?php  } ?>
<script type="text/javascript">
	download_url = "<?php  echo $this->createWeburl('couponmanage', array('op' => 'download'))?>";
	success_url = "<?php  echo $this->createWeburl('couponmanage')?>";
	total = 0;
	angular.module('couponApp', ['app'])
	.controller('CouponDisplayCtrl', ['$scope', '$http', function($scope, $http) {
		$scope.download = function(offset) {
			util.message('正在下载卡券数据...');
			$http.post(download_url, {offset : offset}).success(function(data) {
				if (data.message.errno != 0) {
					util.message(data.message.message, '', 'error');
					return false;
				}
				total += 50;
				if ((data.message.message.pages <= (total/50)) || (!data.message.message.total && !data.message.message.offset)) {
					util.message('同步完成', success_url, 'success');
					return false;
				} else {
					$scope.download(data.message.message.offset);
				}
			});
		};
	}]);
			...
			<div class="panel-body">
				...
				<div class="pull-left alert form form-area" style="width:700px;position:relative;display:block;" id="form1">
					...
					<div class="form-group">
						...
						<div class="col-sm-9 col-xs-12">
							<a href="#modules-modal" class="btn btn-default js-modules-modal" data-toggle="modal">选择模块</a>
